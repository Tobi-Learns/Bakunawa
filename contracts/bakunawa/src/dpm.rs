...
//!
//! The Neutral (regular) mint price = the side's money-share (the crowd
//! forecast). Minting `d` stroops on a side with pre-mint money `m_side` and
//! other-side money `m_other` yields
//!
...
pub fn dpm_shares(m_side: i128, m_other: i128, dollars: i128) -> i128 {
    if m_side <= 0 {
        return dollars; // seed / bootstrap: 1 share = 1 stroop
    }
    let ln_fp = fp_ln_ratio(m_side + dollars, m_side);
    dollars + m_other * ln_fp / LN_SCALE
}
